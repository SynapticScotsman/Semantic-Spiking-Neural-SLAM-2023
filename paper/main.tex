...
% (wiki/analysis/2026-07-29-vsa-query-layer-paper-plan.md, entries cited in
% comments as e.g. [tracker (ax)]) or a named artifact file. Numbers that
% are pending are marked \todo{}.
% =========================================================================
...
% D2 (authorship) is an open decision — placeholder block only.
\title{Algebraic Spatio-Temporal Memory: a Fixed-Size Vector-Symbolic Map
with Exact Merge and a Predictive Capacity Law}

\author{\IEEEauthorblockN{Paul Kirkland\todo{D2: author list and order undecided
--- GC-VSA team, Waterloo (Dumont/Furlong), student contributions}}
\IEEEauthorblockA{Western Sydney University\\
\todo{affiliations, emails, funding acknowledgment}}}

\maketitle

% =========================================================================
\begin{abstract}
Semantic maps on robots are typically growing relational structures: scene
graphs whose state expands with scene content and whose multi-session or
multi-robot merging requires correspondence solving and optimisation. We
study the opposite design point --- a spatial-semantic map whose entire
state is one fixed-size vector. Perceptual observations are bound to
positions by elementwise complex multiplication over Fourier
Holographic Reduced Representation (FHRR) phasors, with continuous space
encoded by fractional power encoding, and superposed into a single trace;
queries are answered by unbinding, in milliseconds on a laptop CPU. Three
properties are measured. \emph{Bounded footprint at benchmark recall:} on
7-Scenes \texttt{chess}, cross-traverse relocalisation from a 32\,KB trace
reaches 0.36\,m median 3D error, against 0.29\,m for exact nearest
neighbour over the same descriptors at 32.8\,MB; product quantisation
cannot reach this size regime (its codebook alone is $\sim$2\,MB).
\emph{Merging is addition:} traces built independently by four robots sum
to the jointly-built trace to within floating-point reordering error
($\sim$10$^{-15}$), with zero data-association decisions, where an
explicit instance-association merge over identical observations degrades
(84\% vs.\ 89\% mean grounding, worst scene 67\%). \emph{A predictive
capacity law:} interference follows
$\chi(N) = a_\mu N + \sqrt{N}\,\sqrt{\alpha/\mathrm{PR} + \gamma/D}\, h(N,\tau)$,
which predicted a class-imbalance failure and its bounded-insertion fix
(grounding 44\%$\to$62\%) and predicts a held-out condition to 1.4\%. On
eight Replica scenes, every grounding miss is attributable to perception:
memory-attributable failures against an explicit-instance ceiling are
zero.
\end{abstract}
...
every downstream query must be answered by traversal code specific to the
structure.

This paper characterises the opposite design point. We use a
vector-symbolic architecture (VSA) as an \emph{algebraic composition
layer}: class, appearance, position, heading, and time are each encoded as
high-dimensional unitary phasor vectors, composed by one binding operator,
and superposed into a map state whose size is fixed \emph{before} the
robot ever moves --- 32\,KB in every experiment here. Insertion is $O(D)$
vector addition. A family of spatial-semantic queries (``where was a
chair?'', ``what is at $(x,y)$?'', ``when was this place occupied?'') is
answered by the algebra's inverse operation --- a single unbind followed
by a kernel readout --- with no per-structure traversal code. Merging two
maps is elementwise addition of two vectors: commutative, associative, and
exact, because the representation makes no association decisions for a
merge step to get wrong.

We are explicit about what is and is not claimed. The memory does not
out-localise a nearest-neighbour search over its own input descriptors,
and a detector-fed object memory cannot label the walls and floors that
dominate dense semantic-segmentation benchmarks; we report both facts with
measurements rather than protocol choices that hide them
(Sec.~\ref{sec:results}). The claims are about what a fixed-size algebraic
map state buys \emph{at the ceiling its perception sets}: recall within
0.07\,m of exact nearest neighbour at $\sim$1000$\times$ fewer bytes on a
public relocalisation benchmark; grounding on eight Replica scenes whose
misses are entirely perceptual (zero memory-attributable); drift bounding
with a filter that runs wholly in the algebra; and a merge operator whose
cost and exactness no growing representation matches.
...
\item \textbf{System} (Sec.~\ref{sec:method}): a fixed-dimensional
  ...
  z-score; or bind a diverse key --- measured substitutes).
...
\textbf{Vector-symbolic architectures.} Holographic reduced
...
symbols in a fixed-dimensional space via binding and superposition, with
capacity analyses assuming i.i.d.\ quasi-orthogonal vectors
...
SSP-SLAM \cite{dumont2023sspslam}, and in grid-cell-inspired form by
GC-VSA \cite{krausse2025gcvsa} (synthetic data). Our contribution to this
...
[Sources in Sec.~\ref{sec:related}; full survey in supplementary.]}
...
An observation at time $t$ --- content vector $c$ (a class atom, an
...
keeps one trace per query family --- VSA ``secondary indexes'':
...
\label{eq:law}
\end{equation}
with three separately-evidenced components [tracker (az); working note in
supplementary]:
\begin{itemize}
\item \textbf{Mean term, exactly linear.} A shared mean direction adds in
  ...
  a diverse bound key phase-scatters it --- the mechanism behind the
  substitution law).
...
[tracker (ba)]. (ii) \emph{The encoder-independent floor}: the
...
\cite{shotton2013scene}: the official train/test split --- memory built
from the 4 training traverses (4{,}000 frames), queried with the 2
physically separate test traverses (2{,}000 frames). (ii) Replica
...
Table~\ref{tab:chess} is the storage/accuracy trade at a fixed frontend
...
z-scored; $D$=8192). 3D error is the retrieved frame's translation error.
[tracker (ax); \texttt{outputs/cross\_recall\_7scenes\_chess.json}]}
\label{tab:chess}
\centering
\small
\begin{tabular}{@{}lrrrr@{}}
...
(Sec.~\ref{sec:conditioning}) --- the claim is scene/encoder-dependent
...
because the trajectory makes heading informative --- the substitution
...
2{,}000 test queries identically [tracker (ax)].
...
(chance 18.2\%) and 78.6\% at $r$=0.5\,m (chance 5.9\%) [tracker (bd)].
...
truth-scored, averaged over the 8 scenes [tracker (bd), (bc)]. The
instance-list row is the scene-graph merge in miniature: 622 greedy
...
24--146\,MB \cite{tian2022kimeramulti} --- different problems solved
(they also estimate the frame; see below), but the right scale bar.

\begin{table}[t]
\caption{4-robot map joining on identical observations, mean over 8
Replica scenes (grounding R@1 at 1\,m, internal protocol).
[\texttt{merge\_comparison\_k4.json} per scene]}
\label{tab:merge}
\centering
\small
\begin{tabular}{@{}lrrrr@{}}
...
[tracker (ah)]: filtered error \textbf{0.364$\pm$0.051\,m} versus dead
...
recall accuracy, which is the mechanism, stated plainly: the honest claim
...
[tracker (ag),(ah); \texttt{vsa\_kalman\_seeds.json}]}
\label{tab:kalman}
\centering
\small
\begin{tabular}{@{}lrr@{}}
...
seeds, 744 queries [tracker (aw)]: raw 2.05$\pm$0.77\,m median, centred
...
\todo{funding, student contributions, collaborator acknowledgments ---
blocked on D2}
