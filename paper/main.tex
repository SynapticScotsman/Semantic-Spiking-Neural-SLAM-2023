...
\captionof{figure}{\textbf{The whole system, working, on public data.}
\textbf{(a)} Two queries from the 7-Scenes \texttt{chess} \emph{test}
walks, beside the frames the 32\,KB trace recalled. They are the
50th- and 75th-percentile errors, not best cases. The memory returns a
stored \emph{pose}, so at larger errors the recalled viewpoint can differ
while the place is still right.
\textbf{(b)} Where every test query lands: the four stored training walks
(grey), each query's true position (dot), and a segment to the position
the trace answered (red). The visible tail is real; the box gives the
median and the fraction within 0.5\,m for the trace and for exact nearest
neighbour over the same descriptors at 1000$\times$ the bytes.
\textbf{(c)} The same algebra answering an object query on Replica
\texttt{room0}: unbinding a class atom returns a spatial belief
\emph{field}, not a lookup. The peak (white cross) lands on a true chair
(green squares mark all ground-truth instances); the unresolved instances
are the identity limitation of Sec.~\ref{sec:limitations}, shown rather
than hidden.}
\label{fig:hero}
\end{strip}
