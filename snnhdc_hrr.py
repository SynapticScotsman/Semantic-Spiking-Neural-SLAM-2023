...
# HRR vectors are real-valued, while the SNN output is spike counts.
# These bounds map each HRR dimension from [-1, 1] into a small spike-count
# range. Increase HRR_MAX_SPIKE_COUNT for a finer analog HRR code.
HRR_MIN_SPIKE_COUNT = 0.0
HRR_MAX_SPIKE_COUNT = 4.0
HRR_COSINE_LOSS_WEIGHT = 0.1
...
@torch.no_grad()
def make_hrr_codebook(num_labels, dimensions, device):
    generator = torch.Generator()
    generator.manual_seed(seed)

    label_role = torchhd.random(1, dimensions, "HRR", generator=generator)[0].to(device)
    class_symbols = torchhd.random(
        num_labels, dimensions, "HRR", generator=generator
    ).to(device)

    # HRR binding uses circular convolution. The SNN is trained to emit
    # ROLE(label) * CLASS_SYMBOL, and inference unbinds ROLE(label).
...
@torch.no_grad()
def hrr_to_spike_counts(hrr_vectors):
    max_abs = hrr_vectors.abs().amax(dim=-1, keepdim=True).clamp_min(1e-8)
    unit_interval = (hrr_vectors / max_abs + 1.0) * 0.5

    return HRR_MIN_SPIKE_COUNT + unit_interval * (
        HRR_MAX_SPIKE_COUNT - HRR_MIN_SPIKE_COUNT
    )


def spike_counts_to_hrr(spike_counts):
    bounded_counts = spike_counts.clamp(
        min=HRR_MIN_SPIKE_COUNT, max=HRR_MAX_SPIKE_COUNT
    )
    unit_interval = (bounded_counts - HRR_MIN_SPIKE_COUNT) / (
        HRR_MAX_SPIKE_COUNT - HRR_MIN_SPIKE_COUNT
    )
    decoded = unit_interval * 2.0 - 1.0
    decoded = F.normalize(decoded, p=2, dim=-1)

    return decoded.as_subclass(torchhd.HRRTensor)
...
class HRRClassifier(nn.Module):
    ...
    def forward(self, encoded_hrr):
        encoded_hrr = torchhd.normalize(encoded_hrr.as_subclass(torchhd.HRRTensor))
        inverse_label_role = self.inverse_label_role.as_subclass(torchhd.HRRTensor)
        class_symbols = self.class_symbols.as_subclass(torchhd.HRRTensor)

        decoded_symbols = torchhd.normalize(torchhd.bind(encoded_hrr, inverse_label_role))
...
@torch.no_grad()
def batch_accuracy_hrr(net, classifier, loader, target_spike_counts, device):
    ...
    for data, targets in iter(loader):
        ...
        batch_target_vectors = target_spike_counts[targets_device]
        loss_val = mse_loss(encoded_spks, batch_target_vectors)
        test_loss_hist.append(loss_val.item())

        encoded_hrr = spike_counts_to_hrr(encoded_spks)
        outputs = classifier(encoded_hrr)
...
def main():
    ...
    snn_name = f"{DIMENSIONS}-D_{beta}-B_HRR_seed({seed})"
    ...
    label_role, class_symbols, class_targets = make_hrr_codebook(
        num_classes, DIMENSIONS, device
    )
    target_spike_counts = hrr_to_spike_counts(class_targets).to(device)

    classifier = HRRClassifier(label_role, class_symbols).to(device)
    ...
    torch.save(
        {
            ...
            "target_spike_counts": target_spike_counts.detach().cpu(),
            "min_spike_count": HRR_MIN_SPIKE_COUNT,
            "max_spike_count": HRR_MAX_SPIKE_COUNT,
        },
        os.path.join(save_dir, "hrr_codebook.pt"),
    )
    ...
    for epoch in range(num_epochs):
        ...
        for data, targets in iter(trainloader):
            ...
            batch_target_vectors = target_spike_counts[targets_device]
            count_loss = mse_loss(encoded_spks, batch_target_vectors)

            encoded_hrr = spike_counts_to_hrr(encoded_spks)
            outputs = classifier(encoded_hrr)
            target_cosines = outputs.gather(1, targets_device.unsqueeze(1)).squeeze(1)
            cosine_loss = 1.0 - target_cosines.mean()

            loss_val = count_loss + HRR_COSINE_LOSS_WEIGHT * cosine_loss
            loss_val.backward()
        ...
        with torch.no_grad():
            net.eval()
            test_acc, test_loss, test_cosine = batch_accuracy_hrr(
                net, classifier, testloader, target_spike_counts, device
            )
